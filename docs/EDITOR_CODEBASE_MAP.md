...
- This is the authoritative reference for the shipped `Carousel Map` system:
  ...
  - reuse of the existing project `prompt_snapshot` + `generate-copy` rewrite path
  - superadmin-only account-scoped stage prompt editing via `editor_account_prompt_overrides`
  - superadmin-only scoped steering memory for generate/regenerate via `carousel_map_generation_steering`
