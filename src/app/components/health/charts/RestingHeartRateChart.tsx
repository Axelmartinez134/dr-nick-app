...
export default function RestingHeartRateChart({ data, hideTrendPill = false }: RestingHeartRateChartProps) {
  ...
  const regressionResult = useMemo(() => {
    ...
    return calculateLinearRegression(valid.map(d => ({ week: d.week, value: d.bpm as number })), xMin, xMax)
  }, [chartData, xMin, xMax])

  const validPointCount = useMemo(() => {
    return chartData.filter(d => typeof d.bpm === 'number' && d.bpm !== null && !Number.isNaN(d.bpm as number)).length
  }, [chartData])
