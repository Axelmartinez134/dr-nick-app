...
type WeeklyLike = { week_number: number; systolic_bp: number | null; date?: string }

interface AliasSystolicMobilePillProps {
  data: WeeklyLike[]
  children: React.ReactNode
}

export default function AliasSystolicMobilePill({ data, children }: AliasSystolicMobilePillProps) {
  const leftMargin = 20
  ...
  const deriveSeries = useMemo(() => {
    return (rows: WeeklyLike[]): AliasMobileValuePillPoint[] => {
      ...
      return sorted.map((r) => ({
        week: r.week_number,
        value: (typeof r.systolic_bp === 'number' && !Number.isNaN(r.systolic_bp) ? Math.round(r.systolic_bp * 100) / 100 : null),
        extra: { date: r.date }
      }))
    }
  }, [])
  ...
  const renderContent = (pt: AliasMobileValuePillPoint) => {
    ...
    return (
      <div className="text-sm">
        ...
        <div className="text-blue-600">{`Systolic: ${pt.value ?? '—'} mmHg`}</div>
        {d.date ? (
