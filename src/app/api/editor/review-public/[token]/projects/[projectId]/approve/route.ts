...
type Resp =
  | { success: true; projectId: string; review_approved: boolean; updated_at: string }
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ token: string; projectId: string }> }) {
  ...
  const next = !!body?.approved;

  const { data, error } = await admin
    .from('carousel_projects')
    .update({ review_approved: next })
    .eq('id', pid)
    .eq('account_id', accountId)
    .is('archived_at', null)
    .select('id, review_approved, updated_at')
    .maybeSingle();
  ...
  return NextResponse.json({
    ...
    review_approved: !!(data as any).review_approved,
