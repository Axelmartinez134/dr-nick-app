...
export default function EditorShell({ initialProjectId = null }: { initialProjectId?: string | null }) {
  ...
  const handleRegularCanvasTextChange = (change: {
    ...
    const resolvedHeadlineRanges = Array.isArray(pendingInlineRanges?.headlineRanges)
      ? pendingInlineRanges.headlineRanges
      : (Array.isArray(curSlide.draftHeadlineRanges) ? curSlide.draftHeadlineRanges : []);
    const resolvedBodyRanges = Array.isArray(pendingInlineRanges?.bodyRanges)
      ? pendingInlineRanges.bodyRanges
      : (Array.isArray(curSlide.draftBodyRanges) ? curSlide.draftBodyRanges : []);
    const prevBodyText = String(
      ((baseInputForMerge && typeof baseInputForMerge === "object" ? (baseInputForMerge as any).body : null) ?? (curSlide.draftBody || ""))
    );
    const nextRegularBodyRanges =
      didEditText && change.lineIndex === 0
        ? remapRangesByDiff({ oldText: prevBodyText, newText: nextBody, ranges: resolvedBodyRanges })
        : resolvedBodyRanges;
    if (didEditText && change.lineIndex === 0 && Array.isArray(nextTextLines?.[0] ? [(nextTextLines as any)[0]] : [])) {
      try {
        const line0 = (nextTextLines as any)?.[0];
        const rebuiltParts = rebuildSingleSpanPartsForLine({
          blockText: nextBody,
          lineText: String(line0?.text || ""),
          hintStart: 0,
        });
        if (rebuiltParts && line0) {
          line0.__sourceParts = rebuiltParts;
          line0.__hintStart = Number(rebuiltParts?.[0]?.sourceStart ?? 0);
          line0.styles = buildTextStylesForLine(rebuiltParts, nextRegularBodyRanges);
        }
      } catch {
        // ignore
      }
      ...
      prev.map((s, i) =>
        i !== slideIndex
          ...
          : {
              ...
              draftHeadlineRanges: resolvedHeadlineRanges,
              draftBodyRanges: nextRegularBodyRanges,
  ...
  // Phase 5E6 (Option 1): bottom panel handlers live in `state.actions` (no separate bottomPanelActions slice).
  const onChangeHeadlineFontSize = (e: any) => {
  ...
  const onClickHeadlineAlign = (a: "left" | "center" | "right") => {
    const nextAlign = a;
    pushUndoSnapshot();
    setSlides((prev: any) =>
      prev.map((s: any, i: number) =>
  ...
  const onChangeHeadlineRichText = (next: any) => {
    try {
      ...
      prev.map((s: any, i: number) =>
        i !== activeSlideIndex
          ...
          : ({
              ...
              inputData:
                s.inputData && typeof s.inputData === "object" ? { ...(s.inputData as any), bodyFontSizePx: nextSize } : s.inputData,
            } as any)
      )
    );
    slidesRef.current = slidesRef.current.map((s: any, i: number) =>
      ...
      prev.map((s: any, i: number) =>
        i !== activeSlideIndex
          ...
          : ({
              ...
              draftBodyTextAlign: nextAlign,
              inputData:
                s.inputData && typeof s.inputData === "object" ? { ...(s.inputData as any), bodyTextAlign: nextAlign } : s.inputData,
            } as any)
      )
    );
    slidesRef.current = slidesRef.current.map((s: any, i: number) =>
  ...
  const onChangeBodyRichText = (next: any) => {
    try {
      const nowProj = currentProjectIdRef.current || "none";
      const nowSlide = activeSlideIndexRef.current + 1;
      const srcProj = currentProjectId || "none";
  ...
  useLayoutEffect(() => {
    editorStore.setState({
      bottomPanelUi: {
        ...
        layoutData,
        inputData,
        layoutHistoryLength: Array.isArray(layoutHistory) ? layoutHistory.length : 0,
        showLayoutOverlays,
        addLog,
